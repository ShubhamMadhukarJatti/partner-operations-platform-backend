...
@Service
@Slf4j
public class AsyncPersonaService {
    ...
    public AsyncPersonaService(EmailService emailService,
    ...
    public void savePersona(PersonaRequest personaRequest,
        ...
        sendCompletionNotification(personaRequest.getOrganizationId());
    }
...
//    }
    ...
    public void callPersona(PersonaRequest personaRequest,
        ...
        log.info("Calling persona API in chunks");
        ...
        List<PersonaModelResponse> allResponses = new ArrayList<>();

        for (int i = 0; i < validSites.size(); i += chunkSize) {

            List<String> chunk = validSites.subList(
                    i,
                    Math.min(i + chunkSize, validSites.size())
            );

            log.info("Processing chunk {} to {}", i, i + chunk.size());

            Map<String, List<String>> postRequest = new HashMap<>();
            postRequest.put("urls", chunk);

            HttpEntity<Map<String, List<String>>> requestEntity =
                    new HttpEntity<>(postRequest);

            ParameterizedTypeReference<List<PersonaModelResponse>> responseType =
                    new ParameterizedTypeReference<>() {};

            try {
                List<PersonaModelResponse> response = restTemplate
                        .exchange(
                                personaUrl,
                                HttpMethod.POST,
                                requestEntity,
        ...
        if (allResponses.isEmpty()) {
            log.warn("Persona API returned empty responses for all chunks");
            return;
        }
